...
(function($) {
	...
	var o = {
		targetGet: function() {
			...
			switch (this.s.targetMode) {
				...
				case "ajax": {
					...
					if (!url) { return; }
					var _uid=this.trigger.data('uid');
					var data=this.getDataByUID(_uid);
					if (!data) {
						o.loading();
						var trigger=this.trigger;
						var ajaxConfig=this.s.ajaxConfig;
						if( ajaxConfig && ajaxConfig.callback){
							$.when(ajaxConfig.callback.apply(null,[])).done(function(){
							   var r=arguments[0].content;
							   var _tdata={};
							   _tdata[_uid]=r;
							   o.ajaxCachePool.push(_tdata); //使用数组来存储，不使用json，为后续的pool size动态调优做铺垫
							   o.cacheData[url] = true;
							   o.target = $('<div class="float_ajax_data">' + r + '</div>');
							   o.targetShow();
							}).fail(function(){
		...
		getDataByUID: function(uid) {
			var _data;
        	$.each(this.ajaxCachePool,function(index,v){
        		if(v[uid]){
        			_data=v[uid];
        			v.shoot?v.shoot++:(v.shoot=1);
        			return false;
        		}
        	});
        	return _data;
		...
		s: {},
		cacheData: {},
		ajaxCachePool:[],// use the uid as the key
		targetProtect: false
	};
